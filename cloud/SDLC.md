...
This is the step-by-step contribution guide for the `cloud/` domain. It mirrors
the repo-wide [`CONTRIBUTING.md`](../CONTRIBUTING.md) but uses this domain's role,
playbook, and collection so you can copy/paste.

**You own this folder.** Pull requests touching `cloud/` are routed to
`@org/team-cloud` automatically by [`.github/CODEOWNERS`](../.github/CODEOWNERS), and
...
| Path | Purpose |
|------|---------|
| `cloud/roles/provision_aws_vm/` | The reusable role (tasks, defaults, meta, molecule scenario) |
| `cloud/playbooks/provision_vm.yml` | Thin wrapper launched by the AAP job template |
| Primary collection | `amazon.aws / azure.azcollection` |
...
git switch main
git pull --ff-only
git switch -c feature/cloud-add-spot-instances
```

## 3. Make the change

Edit the role under `cloud/roles/provision_aws_vm/`:

```
cloud/roles/provision_aws_vm/tasks/main.yml        # the logic
cloud/roles/provision_aws_vm/defaults/main.yml     # variable defaults
cloud/roles/provision_aws_vm/molecule/default/     # the test that proves it works
```
...
> Cloud team calls behind `cloud_check_mode`. In production that defaults to `false` so the
...
cd cloud/roles/provision_aws_vm
molecule test
cd -
```
...
```bash
git add cloud/
git commit -m "cloud: <short description of the change>"
git push -u origin feature/cloud-add-spot-instances
```

Open the PR against `main`. The moment you do:

- **CI runs automatically** — `yamllint` + `ansible-lint` repo-wide, and
  `molecule test` for the `cloud` job (path-filtered; other domains don't run).
- **CODEOWNERS requests `@org/team-cloud`** because you changed files under `cloud/`.
...
- A member of **Cloud team** reviews — they're accountable for what runs in this domain.
...
| You want to… | Do this |
|--------------|---------|
| Start work | `git switch -c feature/cloud-add-spot-instances` |
| Check style | `yamllint .` |
| Check best practices | `ansible-lint` |
| Test this role | `cd cloud/roles/provision_aws_vm && molecule test` |
| Get it reviewed | Open a PR — `@org/team-cloud` is requested automatically |
