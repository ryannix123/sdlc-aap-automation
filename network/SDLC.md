...
This is the step-by-step contribution guide for the `network/` domain. It mirrors
the repo-wide [`CONTRIBUTING.md`](../CONTRIBUTING.md) but uses this domain's role,
playbook, and collection so you can copy/paste.

**You own this folder.** Pull requests touching `network/` are routed to
`@org/team-network` automatically by [`.github/CODEOWNERS`](../.github/CODEOWNERS), and
...
| Path | Purpose |
|------|---------|
| `network/roles/manage_dns_record/` | The reusable role (tasks, defaults, meta, molecule scenario) |
| `network/playbooks/manage_dns_record.yml` | Thin wrapper launched by the AAP job template |
| Primary collection | `infoblox.nios_modules` |
...
Edit the role under `network/roles/manage_dns_record/`:

```
network/roles/manage_dns_record/tasks/main.yml        # the logic
network/roles/manage_dns_record/defaults/main.yml     # variable defaults
network/roles/manage_dns_record/molecule/default/     # the test that proves it works
```
...
> Network team calls behind `dns_check_mode`. In production that defaults to `false` so the
...
```bash
git add network/
git commit -m "network: <short description of the change>"
...
- **CI runs automatically** — `yamllint` + `ansible-lint` repo-wide, and
  `molecule test` for the `network` job (path-filtered; other domains don't run).
- **CODEOWNERS requests `@org/team-network`** because you changed files under `network/`.
...
- A member of **Network team** reviews — they're accountable for what runs in this domain.
...
| You want to… | Do this |
|--------------|---------|
| Start work | `git switch -c feature/network-add-cname-support` |
| Check style | `yamllint .` |
| Check best practices | `ansible-lint` |
| Test this role | `cd network/roles/manage_dns_record && molecule test` |
| Get it reviewed | Open a PR — `@org/team-network` is requested automatically |
