# SDLC — Server platform teams

This is the step-by-step contribution guide for the `servers/` domain. It mirrors
the repo-wide [`CONTRIBUTING.md`](../CONTRIBUTING.md) but uses this domain's role,
playbook, and collection so you can copy/paste.

**You own this folder.** Pull requests touching `servers/` are routed to
`@org/team-windows @org/team-linux` automatically by [`.github/CODEOWNERS`](../.github/CODEOWNERS), and
...
| Path | Purpose |
|------|---------|
| `servers/roles/configure_baseline/` | The reusable role (tasks, defaults, meta, molecule scenario) |
| `servers/playbooks/configure_baseline.yml` | Thin wrapper launched by the AAP job template |
| Primary collection | `ansible.windows / rhel_system_roles` |
...
git switch main
git pull --ff-only
git switch -c feature/servers-add-firewall-baseline
```

## 3. Make the change

Edit the role under `servers/roles/configure_baseline/`:

```
servers/roles/configure_baseline/tasks/main.yml        # the logic
servers/roles/configure_baseline/defaults/main.yml     # variable defaults
servers/roles/configure_baseline/molecule/default/     # the test that proves it works
```
...
> Server platform teams calls behind `servers_check_mode`. In production that defaults to `false` so the
...
cd servers/roles/configure_baseline
molecule test
cd -
```
...
```bash
git add servers/
git commit -m "servers: <short description of the change>"
git push -u origin feature/servers-add-firewall-baseline
```

Open the PR against `main`. The moment you do:

- **CI runs automatically** — `yamllint` + `ansible-lint` repo-wide, and
  `molecule test` for the `servers` job (path-filtered; other domains don't run).
- **CODEOWNERS requests `@org/team-windows @org/team-linux`** because you changed files under `servers/`.
...
- A member of **Server platform teams** reviews — they're accountable for what runs in this domain.
...
| You want to… | Do this |
|--------------|---------|
| Start work | `git switch -c feature/servers-add-firewall-baseline` |
| Check style | `yamllint .` |
| Check best practices | `ansible-lint` |
| Test this role | `cd servers/roles/configure_baseline && molecule test` |
| Get it reviewed | Open a PR — `@org/team-windows @org/team-linux` is requested automatically |
