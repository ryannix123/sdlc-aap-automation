...
This is the step-by-step contribution guide for the `storage/` domain. It mirrors
the repo-wide [`CONTRIBUTING.md`](../CONTRIBUTING.md) but uses this domain's role,
playbook, and collection so you can copy/paste.

**You own this folder.** Pull requests touching `storage/` are routed to
`@org/team-storage` automatically by [`.github/CODEOWNERS`](../.github/CODEOWNERS), and
...
| Path | Purpose |
|------|---------|
| `storage/roles/provision_ontap_volume/` | The reusable role (tasks, defaults, meta, molecule scenario) |
| `storage/playbooks/provision_volume.yml` | Thin wrapper launched by the AAP job template |
| Primary collection | `netapp.ontap` |
...
git switch main
git pull --ff-only
git switch -c feature/storage-add-snapshot-policy
```

## 3. Make the change

Edit the role under `storage/roles/provision_ontap_volume/`:

```
storage/roles/provision_ontap_volume/tasks/main.yml        # the logic
storage/roles/provision_ontap_volume/defaults/main.yml     # variable defaults
storage/roles/provision_ontap_volume/molecule/default/     # the test that proves it works
```
...
> Storage team calls behind `storage_check_mode`. In production that defaults to `false` so the
...
cd storage/roles/provision_ontap_volume
molecule test
cd -
```
...
```bash
git add storage/
git commit -m "storage: <short description of the change>"
git push -u origin feature/storage-add-snapshot-policy
```

Open the PR against `main`. The moment you do:

- **CI runs automatically** — `yamllint` + `ansible-lint` repo-wide, and
  `molecule test` for the `storage` job (path-filtered; other domains don't run).
- **CODEOWNERS requests `@org/team-storage`** because you changed files under `storage/`.
...
- A member of **Storage team** reviews — they're accountable for what runs in this domain.
...
| You want to… | Do this |
|--------------|---------|
| Start work | `git switch -c feature/storage-add-snapshot-policy` |
| Check style | `yamllint .` |
| Check best practices | `ansible-lint` |
| Test this role | `cd storage/roles/provision_ontap_volume && molecule test` |
| Get it reviewed | Open a PR — `@org/team-storage` is requested automatically |
